  ...
  describe('createUser', () => {
    ...
    afterEach(() => {    
     ...
     it('should return error(s) when validation fails', async () => {
      const req = {
        body: { username: 123, email: faker.internet.email(), password: 'sdf' } //the name  and password are invalid
      };
    ...
    it('should not create an user, due to db error', async () => {
      ...
      const req = {
        body: { username: faker.name.findName(), email: faker.internet.email(), password: faker.internet.password() }
      };
